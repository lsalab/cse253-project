...
def get_apdu(typeASDU: int, ioa: int, dato: float, tx: int, rx: int, causeTx:int =1) -> bytes: 
    ...
    if typeASDU == 36:
        ...
        # apci = APCI(ApduLen=APDULEN_36, Type=0x00,Tx=7, Rx=15)
        # apci = APCI(ApduLen=APDULEN_36, Apci=ApciTypeI(Tx=tx, Rx=rx))
        # asdu = ASDU(TypeId=typeASDU, SQ=0, NumIx=1, CauseTx=1, Test=0, OA=1, Addr=2, IOA=IOA36(IOA=ioa, Value=dato, QDS=QDS(BL=0, SB=0, NT=0, IV=0), CP56Time=cp56time()))
    elif typeASDU == 3:
